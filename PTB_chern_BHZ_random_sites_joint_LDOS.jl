...
# julia PTB_chern_BHZ_new_random_joint_LDOS.jl t0=1.0 t=1.0 m_0=1.0 x_periodic=1 y_periodic=1 L=40 proportion_sites=0.33
t0 = 1.0
t = 1.0
m_0 = 1.0
x_periodic = 1
...
output_root = "data/chern_BHZ"
seed = nothing
run_id = nothing

function parse_command_line_overrides!()
    global t0, t, m_0, x_periodic, y_periodic, L, proportion_sites, output_root, seed, run_id
...
function find_random_non_nn_sites(N_target, Lx, Ly, points_x_array, points_y_array)
    ...
    PTB_index_bigger = shuffle(shuffle(PTB_index_bigger))
...
"""Return the local Chern marker, summed over the two orbitals at each PTB site."""
...
function calculate_bott_index(t0, t, m_0, x_periodic, y_periodic, L, proportion_sites)
    Lx = L
    Ly = L
...
function output_filename(t0, t, m_0, x_periodic, y_periodic, L, proportion_sites, run_id)
    timestamp = Dates.format(now(), "yyyy-mm-dd_HHMMSS")
    stem = "t0=$(t0)_t=$(t)_m_0=$(m_0)_x_periodic=$(x_periodic)_y_periodic=$(y_periodic)_L=$(L)_proportion_sites=$(proportion_sites)"
    if run_id !== nothing
...
function main()
    ...
    filename = output_filename(t0, t, m_0, x_periodic, y_periodic, L, proportion_sites, run_id)
